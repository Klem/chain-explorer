...
export enum ProviderRPC {
    ALCHEMY = 'PROVIDER_ALCHEMY_RPC',
    ETHERSCAN = 'PROVIDER_ETHERSCAN_RPC',
}

type Network = 'ETH_MAIN_RPC' | 'ETH_TEST_RPC' | 'ETH_LOCAL_RPC';
type Provider = ProviderRPC.ALCHEMY | ProviderRPC.ETHERSCAN

const network = (process.env.ETH_SELECTED_RPC || 'ETH_LOCAL_RPC') as Network; // fallback, requires local node
const provider = (process.env.PROVIDER_SELECTED_RPC || ProviderRPC.ALCHEMY) as Provider;
